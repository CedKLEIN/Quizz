...
public class RuleEngineServiceTests
{
    ...
    public void EvaluateConditions_Equals()
    {
        ...
        var nodesFiltered = nodes
            .Where(n => ruleEngineService.EvaluateConditions(nodeConditions.Conditions, n))
            .ToList();
        Assert.That(nodesFiltered, Is.Not.Null);
        Assert.That(nodesFiltered, Has.Count.EqualTo(2));
        Assert.That(
            nodesFiltered.Select(n => n.Question),
    ...
    public void EvaluateConditions_Complex()
    {
        ...
        Assert.That(nodesFiltered.Select(n => n.Question), Is.EquivalentTo(new[] { "Allemagne" }));
    }
}
